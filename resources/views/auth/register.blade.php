...
@section('content')
    <div class="w-full max-w-md mx-auto px-4">
        <div class="text-center mb-8">
            ...
            <p class="text-navy-200 text-sm mt-1">Buat Akun Baru</p>
        </div>
        <div class="bg-white/10 backdrop-blur-xl border border-white/20 rounded-3xl shadow-2xl p-8">
            <h2 class="text-xl font-bold text-white mb-6">{{ __('Register') }}</h2>
            <form method="POST" action="{{ route('register') }}" class="space-y-5">
                @csrf
                <div class="space-y-2">
                    <label for="name" class="block text-sm font-semibold text-navy-100">{{ __('Name') }}</label>
                    <input id="name" type="text" name="name" value="{{ old('name') }}" autocomplete="name" autofocus
                        class="block w-full px-4 py-3 bg-white/10 border @error('name') border-danger-400 @else border-white/20 @enderror rounded-xl text-white placeholder-navy-300 text-sm focus:ring-2 focus:ring-gold-400 focus:border-gold-400 focus:bg-white/15 focus:outline-none transition-all"
                        placeholder="Nama lengkap">
                    @error('name')<p class="text-sm text-danger-300">{{ $message }}</p>@enderror
                </div>
                <div class="space-y-2">
                ...
                <div class="space-y-2">
                    ...
                    <input id="password-confirm" type="password" name="password_confirmation" autocomplete="new-password"
                        class="block w-full px-4 py-3 bg-white/10 border border-white/20 rounded-xl text-white placeholder-navy-300 text-sm focus:ring-2 focus:ring-gold-400 focus:border-gold-400 focus:bg-white/15 focus:outline-none transition-all"
                        placeholder="••••••••">
                </div>
                ...
                <div class="text-center">
                    <a href="{{ route('login') }}" class="text-sm text-gold-300 hover:text-white transition-colors font-medium">Sudah punya akun? Login</a>
                </div>
            </form>
        </div>
    </div>
