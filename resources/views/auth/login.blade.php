                ...
                <div class="space-y-2" x-data="{ showPassword: false }">
                    ...
                    <div class="relative">
                        <input id="password" :type="showPassword ? 'text' : 'password'" name="password" autocomplete="current-password"
                            class="block w-full px-4 py-3 pr-12 bg-white/10 border @error('password') border-danger-400 @else border-white/20 @enderror rounded-xl text-white placeholder-navy-300 text-sm focus:ring-2 focus:ring-gold-400 focus:border-gold-400 focus:bg-white/15 focus:outline-none transition-all"
                            placeholder="••••••••">
                        <button type="button" @click="showPassword = !showPassword" class="absolute inset-y-0 right-0 flex items-center pr-3 text-navy-300 hover:text-white transition-colors">
