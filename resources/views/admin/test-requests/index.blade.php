                        ...
                        <tbody class="divide-y divide-slate-50 text-sm">
                            @forelse($requests as $req)
                                <tr class="hover:bg-slate-50/80 transition-colors">
                                    ...
                                    <td class="px-6 py-4 text-slate-500 text-xs">{{ $req->created_at->format('d M Y H:i') }}
                                    </td>
                            ...
                            @empty
                                <tr>
                                    <td colspan="7" class="px-6 py-12 text-center text-slate-400">Tidak ada permohonan
                                        pengujian.
                                    </td>
                                </tr>
                            @endforelse
                    ...
                    @forelse($requests as $req)
                        <div class="p-4 block hover:bg-slate-50 transition-colors">
                            <div class="flex items-center justify-between mb-2">
                                <span
                                    class="font-mono font-bold text-slate-800 text-xs bg-slate-100 px-2 py-0.5 rounded border border-slate-200">{{ $req->request_code }}</span>
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-[10px] font-semibold shrink-0
                                                {{ $req->status === 'pending_payment' ? 'bg-yellow-100 text-yellow-800' : '' }}
                                                {{ $req->status === 'payment_uploaded' ? 'bg-blue-100 text-blue-800' : '' }}
                                                {{ $req->status === 'payment_verified' ? 'bg-indigo-100 text-indigo-800' : '' }}
                                                {{ $req->status === 'in_testing' ? 'bg-violet-100 text-violet-800' : '' }}
                            ...
                            <h4 class="text-sm font-bold text-slate-800 mb-1">{{ $req->user->name ?? '-' }}</h4>
                            <div class="flex flex-wrap gap-2 text-xs text-slate-500 mb-3">
                                <span class="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-semibold
                                                ...
                                                {{ $req->sample_type === 'jaringan_tanaman' ? 'bg-green-100 text-green-700' : '' }}
                                            ">{{ $req->sample_type_label }}</span>
                                <span class="font-semibold text-slate-700">📦 {{ $req->num_samples }} Sampel</span>
                                <span class="text-slate-400">🕒 {{ $req->created_at->format('d M Y H:i') }}</span>
                            </div>
                            <div class="flex justify-end pt-2 border-t border-slate-50">
                                <a href="{{ route('admin.test-requests.show', $req) }}"
                                    class="inline-flex items-center px-4 py-1.5 text-xs font-semibold text-cyan-700 bg-cyan-50 rounded-lg hover:bg-cyan-100">Lihat
                                    Detail</a>
                            </div>
                        </div>
                    @empty
                        <div class="p-8 text-center text-slate-400 text-sm">Tidak ada permohonan pengujian.</div>
                    @endforelse
                </div>
