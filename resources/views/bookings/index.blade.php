...
@section('content')
    <div class="space-y-8 animate-fade-in">
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4 bg-white/5 backdrop-blur-md border border-white/10 rounded-2xl p-6 shadow-xl relative overflow-hidden">
            <div class="absolute -top-24 -right-24 w-64 h-64 bg-cyan-500/20 rounded-full mix-blend-screen filter blur-[80px]"></div>
            ...
            <div class="relative z-10">
                <a href="{{ route('bookings.create') }}" class="inline-flex items-center px-5 py-2.5 text-sm font-semibold text-white bg-gradient-to-r from-cyan-600 to-teal-600 rounded-xl hover:from-cyan-500 hover:to-teal-500 transition-all shadow-lg hover:shadow-cyan-500/30 transform hover:-translate-y-0.5">
        ...
        <div class="bg-white rounded-3xl border border-slate-100 shadow-xl shadow-slate-200/40 overflow-hidden">
            <div class="overflow-x-auto">
                <table class="w-full text-left min-w-max">
                    <thead><tr class="bg-slate-50 border-b border-slate-100 text-slate-500 text-xs font-bold uppercase tracking-widest">
                    ...
                    <tbody class="divide-y divide-slate-50 text-sm">
                        @forelse($bookings as $i => $booking)
                            <tr class="hover:bg-slate-50/80 transition-colors">
                                ...
                                <td class="px-6 py-4"><span class="font-semibold text-slate-800">{{ $booking->room->name }}</span><span class="block text-xs text-slate-400 font-mono">{{ $booking->room->code }}</span></td>
                                ...
                                <td class="px-6 py-4 text-center">
                                    <a href="{{ route('bookings.show', $booking) }}" class="inline-flex items-center px-3 py-1.5 text-xs font-semibold text-cyan-600 bg-cyan-50 border border-cyan-100 rounded-lg hover:bg-cyan-100 transition-colors">Detail</a>
                                </td>
                            </tr>
                        @empty
                            <tr><td colspan="7" class="px-8 py-16 text-center">
                                ...
                                <a href="{{ route('bookings.create') }}" class="inline-flex items-center px-4 py-2 text-sm font-semibold text-cyan-600 bg-cyan-50 rounded-xl hover:bg-cyan-100 transition-colors">
